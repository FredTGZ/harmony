<?php
	...
	class Plot {
		...
		protected $TextFontSize = 8;
		protected $TextFontColor = null;
		protected $TextFontFamily = "Sans Serif";
		protected $TitleFontSize = 20;
		protected $TitleFontColor = null;
		protected $TitleFontFamily = "Sans Serif";


		// Style properties
		protected $Title = "";
		protected $TitleHeight;		// Fixed title height in pixels
		protected $TitlePadding;	// Padding of the title area
		protected $TitleArea;		// Coordinates of the title area
		...
		public function printTitle() {
			...
			/*$this->text->FontSize = $this->TitleFontSize;
			$this->text->FontColor = $this->TitleFontColor;
			$this->text->printCentered($this->img, $yCenter, $this->Title, $this->text->fontCondensedBold);
			$this->text->FontSize = $this->TextFontSize; 
			$this->text->FontColor = $this->TextFontColor;*/
			
			$this->TitleText->printCentered($this->img, $yCenter, $this->Title);
		}
		...
		{
			$this->ConfigFile = new CConfigFile($file);
			$vars = $this->ConfigFile->GetVarArray();
			
			$this->TextFontFamily = $vars['TextFontFamily'];
			$this->TextFontSize = $vars['TextFontSize'];
			$this->TextFontColor = new Color($vars['TextFontColor']);

			// Loading font definitions for title			
			$this->TitleText->SetFontFamily($vars['TitleFontFamily']);
			$this->TitleText->SetFontSize($vars['TitleFontSize']);
			$this->TitleText->SetFontColor(new Color($vars['TitleFontColor']));

			$this->backGroundColor = new Color($vars['BackgroundColor']);

			$this->text->SetFontSize($this->TextFontSize); 
			$this->text->SetFontColor($this->TextFontColor);
			
			// Loading Palettes
			$this->LoadPalette($vars, 'PieChartColor', 'pieColorSet', 0.7);
			$this->LoadPalette($vars, 'LineChartColor', 'lineColorSet', 0.7);
			$this->LoadPalette($vars, 'BarChartColor', 'barColorSet', 0.75);
			$this->LoadColorList($vars, 'BackgroundChartColor', 'aquaColor');
			$this->LoadColorList($vars, 'AxisChartColor', 'axisColor');
		}

		protected function LoadColorList($VarsArray, $VarName, $ListName)
		{
			eval('$this->Palette->'.$ListName.' = array(); $i = 1;');

			while(isset($VarsArray[$VarName.$i]))
				eval('$this->Palette->'.$ListName.'[] = new Color($VarsArray[$VarName.($i++)]);');
		}
		
		protected function LoadPalette($VarsArray, $VarName, $PaletteName, $ShadowFactor)
		{
			$colors = array();
			$i = 1;

			while(isset($VarsArray[$VarName.$i]))
				$colors[] = new Color($VarsArray[$VarName.($i++)]);

			eval('$this->Palette->'.$PaletteName.'= new ColorSet($colors,'.$ShadowFactor.');');
		}
	}
?>
