<?php
	...
	class Color {
		 ...
		 */
		public function Color($red=-1, $green=-1, $blue=-1, $alpha = 0) {
			$this->red = (int) 0;
			$this->green = (int) 0;
			$this->blue = (int) 0;			
			$this->alpha = (int) round(127.0 / 255);
		...
		private function ConvertHexa($rgb)
		{
			if (strlen($rgb)==6 || (strlen($rgb)==7)) {
				if (strlen($rgb)==7) $pos = 1;
				else $pos=0;
			
				$this->red = hexdec(substr($rgb, $pos, 2));
				$this->green = hexdec(substr($rgb, $pos+2, 2));
				$this->blue = hexdec(substr($rgb, $pos+4, 2));
			}
			else die($rgb." is not a valid format");			
		}
